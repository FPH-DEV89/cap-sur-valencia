# Consignes — Actualité hebdomadaire Cap sur Valencia (2026-08-24)
...
1. **Priorité temporelle** : un événement daté familial (fête, festival, exposition temporaire, marché, foire) se déroulant à **≤ 10 jours** de la date du jour (2026-08-24). Un événement passé ou dans plus de 6 semaines est STRICTEMENT écarté.
...
2. Copie le modèle `/tmp/cap-sur-valencia/actualites/2026-08-16-bioparc-valencia.html` vers `/tmp/cap-sur-valencia/actualites/2026-08-24-<slug>.html`.
...
3. **`sitemap.xml`** : Ajouter l'entrée `<url>` correspondante (`https://capsurvalencia.fr/actualites/2026-08-24-<slug>`).

## CONTRAINTES DE COMMANDE
- Ne modifier QUE ces fichiers (`actualites/2026-08-24-<slug>.html`, `assets/img/...` si nouvelle photo, `pages/actualites.html`, `feed.xml`, `sitemap.xml`).
